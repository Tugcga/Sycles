	...
	const XSI::CTime& eval_time)
{
	...
	{// there is connection with shader parameter 
		...
		ccl::ShaderNode* source_node = xsi_node_to_cycles(xsi_source_node, shader_graph, nodes_map, eval_time);
		if (source_node != NULL)
		{
	...
	const XSI::CTime& eval_time)
{
	...
	bool is_connect = sync_shader_parameter_connection(xsi_finall_parameter, cycles_name, cycles_node, shader_graph, nodes_map, eval_time);
	...
	const XSI::CTime& eval_time)
{
	...
	else
	{
		...
		else
		{
			...
			return sync_cycles_shader(xsi_shader, shader_type, params, eval_time, shader_graph, nodes_map);
		}
	...
	const XSI::CTime& eval_time)
{
	...
	if (material_port_source.IsValid())
	{
		...
		if (xsi_node.IsValid())
		{
			ccl::ShaderNode* to_return = xsi_node_to_cycles(xsi_node, shader_graph, nodes_map, eval_time);
			if (to_return != NULL)
			{
...
ccl::ShaderGraph* material_to_graph(const XSI::Material& xsi_material, const XSI::CTime& eval_time)
{
	...
	ccl::ShaderNode* surface_node = sync_material_port(shader_graph, xsi_material_parameters.GetItem("surface"), nodes_map, xsi_node_surface_port_name, xsi_node_surface_prog_id, eval_time);
	
	XSI::CString xsi_node_volume_port_name = "";
	XSI::CString xsi_node_volume_prog_id = "";
	ccl::ShaderNode* volume_node = sync_material_port(shader_graph, xsi_material_parameters.GetItem("volume"), nodes_map, xsi_node_volume_port_name, xsi_node_volume_prog_id, eval_time);

	XSI::CString xsi_node_displace_port_name = "";
	XSI::CString xsi_node_displace_prog_id = "";
	ccl::ShaderNode* displacement_node = sync_material_port(shader_graph, xsi_material_parameters.GetItem("normal"), nodes_map, xsi_node_displace_port_name, xsi_node_displace_prog_id, eval_time);
...
int sync_material(ccl::Scene* scene, const XSI::Material &xsi_material, const XSI::CTime &eval_time)
{
...
ccl::ShaderGraph* shaderball_shadernode_to_graph(const XSI::Shader& xsi_shader, bool is_surface, const XSI::CTime& eval_time, bool &out_success)
{
	...
	std::unordered_map<ULONG, ccl::ShaderNode*> nodes_map;
	ccl::ShaderGraph* shader_graph = new ccl::ShaderGraph();
	ccl::ShaderNode* shader_node = xsi_node_to_cycles(xsi_shader, shader_graph, nodes_map, eval_time);
