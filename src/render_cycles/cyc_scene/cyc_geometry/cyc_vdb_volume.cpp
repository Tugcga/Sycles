...
void add_vdb_to_volume(ccl::Scene* scene, ccl::Volume* volume_geom, const XSI::CTime& eval_time, const XSI::X3DObject& xsi_object, VDBData* vdb_data, ULONG index, int frame, const XSI::CString& file_path,
...
void sync_vdb_volume_geom_process(ccl::Scene* scene, ccl::Volume* volume_geom, UpdateContext* update_context, XSI::X3DObject& xsi_object, VDBData* vdb_data, const XSI::CString &file_path)
{
	if (!vdb_data) {
		return;
	}
