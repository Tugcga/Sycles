...
XSIImageLoader::XSIImageLoader(XSI::ImageClip2& xsi_clip, const XSI::CTime& eval_time)
{
	...
	XSI::CParameterRefArray clip_params = xsi_clip.GetParameters();
	XSI::Parameter color_prof_param(clip_params.GetItem("RenderColorProfile"));
	m_color_profile = color_prof_param.GetValue(eval_time);
}
...
bool XSIImageLoader::load_metadata(const ccl::ImageDeviceFeatures& features, ccl::ImageMetaData& metadata)
{
	...
	metadata.depth = 1;
	// TODO: check that normal map (and environment hdr) rendered properly with valid colo space
	metadata.colorspace = m_color_profile == "sRGB" ? ccl::u_colorspace_srgb : (m_color_profile == "Linear" ? ccl::u_colorspace_raw : ccl::u_colorspace_auto);
...
bool XSIImageLoader::equals(const ImageLoader& other) const
{
	...
	return m_xsi_clip_id == other_loader.get_clip_id();
}

int XSIImageLoader::get_tile_number() const
{
	// TODO: supports different tile number
	return 0;
}